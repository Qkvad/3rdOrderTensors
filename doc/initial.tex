...
\begin{defn} Let $\mathcal{A} \in \mathbb{R}^{l \times p \times n}$ and $\mathcal{B} \in \mathbb{R}^{p \times m \times n}$. The t-product $\mathcal{A} \ast \mathcal{B}$\footnote{our Octave implementation of t-product is called \code{tproduct}.} is a $l \times m \times n$ tensor defined as
